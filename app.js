  ...
  new Rellax('.rellax');
    ...
    hoverArea.addEventListener("mousemove", function(event) {
      ...
      if (distance >= 90) { // Проверяем, превысило ли перемещение 3 см (90 пикселей)
        ...
        "Ромб — параллелограмм, у которого все стороны равны.","Прямая — ","S=vt","принУдить","Простое число делится на 1 и само себя"]; // Пример массива с несколькими элементами
        numbers = shuffleArray(numbers); // Перемешиваем массив перед использованием
